...
const SodEodController = {
    addSodEod: asyncHandler(async (req, res) => {
        ...
        const userFound = await User.findById(req.userAuth._id).select("workDetails");
        ...
        return ApiResponse.success(res, `${type.toUpperCase()} Recorded`, todaysEntry);
    }),

    getAllSodEod: asyncHandler(async (req, res) => {
        const userId = req.userAuth._id;
        userFound = await User.findById(userId).select("workDetails").populate("workDetails");
        if (!userFound) {
